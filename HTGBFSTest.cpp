...
//HGBFS test::murmurhash set len=3 , tested on ER graph with 100  MCR queries
...
int  createsupergraph(vector<int> sgadj[])
{
	...
	ifstream myfile ("/home/bbv1814/Desktop/j2/graphs/egv1/egv1superg.txt");
	string line;	
...
int findSuperVertex(int v)
{
	...
	ifstream myfile ("/home/bbv1814/Desktop/j2/graphs/egv1/egv1cluster.txt");
	string line;	
	if (myfile.is_open())                     //if the file is open
  	{
    		while (! myfile.eof() )                 //while the end of file is NOT reached
...
bool BFSG(vector<int> adj[], long graph_hash[], string constraints, int src, int dest, int v, int pred[], int dist[], MYSQL *conn,vector<int> sgadj[],int k) 
{ 
...
int printShortestDistanceg(vector<int> adj[], long graph_hash[], string constraints, int s, int dest, int v, MYSQL *conn,vector<int> sgadj[],int k) 
{ 

	int pred[v], dist[v]; 

	//cout<<"in printshortestdistance() function"<<endl;
	if (BFSG(adj, graph_hash, constraints, s, dest, v, pred, dist, conn,sgadj,k) == false) 
	{ 
	...
	if (myfile2.is_open())                     //if the file is open
  	{
			...
			{ 
				...
				constraint=ar[2];			
				clock_t start=clock();
				int d=printShortestDistanceg(adj, graph_hash, constraint, source, destination, number_of_vertices, conn,sgadj,k);  
				clock_t stop=clock();
